...
def src():
    ...
    if (panel.filename)=='':
        entry_box.insert(0,'no_Image_selected')
...
def secscreen():
    ...
    b2.place(x=1300,y=0)
    #label4 = Label(bottom,text="Detect Image after detection",fg='gray17',bg="white",relief="solid",font=("arial",16,""))
    #label4.place(x=900,y=10)
    ...
    Toplevel.mainloop()
    #---------finalimage--------
##    imge1=Image.open(img)
##    sizes1=(1200,580)
##    imge1.thumbnail(sizes1)
##    photo1=ImageTk.PhotoImage(imge1)
##    label7=Label(bottom,image=photo1)
##    label7.place(x=900,y=40)
##    Toplevel.mainloop()
def imgpt():
    a=entry_box.get()
    return a
...
bottom.pack(fill=X)
 ...
 #-----------top fram design----------
label1 = Label(top,text="OBJECT DETECTION",fg='black',bg='dark violet',relief="solid",font=("stencil",55,"bold"))
label1.pack(fill=BOTH,pady=4,padx=4)
label2 = Label(bottom,text="select image for detection",fg='gray17',bg="white",relief="solid",font=("arial",16,"")).place(x=717,y=272)
b1 = Button(bottom,text = "Upload_Image",relief=RAISED,font=("arial",30,'bold'),command=lambda :src())
b1.place(x=400,y=270)
#--------entry_box---------
entry_box=Entry(font='verdana 14 bold', width=40,bd=6)
entry_box.insert(0,'')
entry_box.place(x=716,y=400)
    
b2 = Button(bottom,text="Exit",relief=RAISED,font=("arial",12,'bold'),command = panel.destroy)
b2.place(x=1300,y=101)
b1 = Button(bottom,text = "Show Uploaded Image",relief=RAISED,font=("arial",30,'bold'),command=secscreen)
b1.place(x=400,y=360)
b1 = Button(bottom,text = "Show Detected Image",relief=RAISED,font=("arial",30,'bold'),command=run)
